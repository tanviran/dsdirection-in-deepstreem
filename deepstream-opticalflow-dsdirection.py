    ...
    while l_frame is not None:
        ...
        while l_obj:
            try:
                ...
                #print(obj_meta.output)
                #obj_meta.text_params.display_text = f"ID:{obj_meta.object_id}\nConf:{obj_meta.confidence: .2f}"#\nSpeed={user_meta.output.object.speed:.2f}"#, Y = {y_mean:.2f}\nSpeed={speed:.2f}\n{direction}"
                #l_user = frame_meta.frame_user_meta_list
                l_user = obj_meta.obj_user_meta_list
                #print(f"LUSER{l_user}")
                while l_user:
                    
                    try:
                        user_meta = pyds.NvDsUserMeta.cast(l_user.data) 
                        if user_meta.base_meta.meta_type == pyds.nvds_get_user_meta_type("NVIDIA.NVDSDIRECTION.DIR_META"):             
                            #user_meta_data = pyds.DsDirectionOutput(user_meta.user_meta_data)
                            user_meta_data = pyds.DsDirectionOutput.cast(user_meta.user_meta_data)
                            # Access the flowx, flowy, speed, and direction attributes
                            flowx = user_meta_data.object.flowx
                            flowy = user_meta_data.object.flowy
                            speed = user_meta_data.object.speed
                            direction = user_meta_data.object.direction

                            # Print the flowx, flowy, speed, and direction attributes
                            print(f"Object {obj_meta.object_id}:")
                            print(f"\tflowx: {flowx}")
                            print(f"\tflowy: {flowy}")
                            print(f"\tspeed: {speed}")
                            print(f"\tdirection: {direction}")
                            #print(user_meta_data)
                            #\nX = {flowx:.2f}Y = {flowy:.2f}\nSpeed={speed:.2f},
                            obj_meta.text_params.display_text = f"ID:{obj_meta.object_id}\tDir={direction}"
...
def main(args):
    ...
    print("Creating dsdirection \n")
    dsdirection = Gst.ElementFactory.make("dsdirection", "nvdsdirection")
    if not dsdirection:
        sys.stderr.write("Unable to create dsdirection\n")
    ...
    nvvidconv = Gst.ElementFactory.make("nvvideoconvert", "convertor2")
    if not nvvidconv:
    ...
    pgie.link(tracker)
    tracker.link(nvof)
    nvof.link(dsdirection)
    dsdirection.link(tiler)
